...
{
    public class Chapter1Module2Model : PageModel
    {
        public string error = "";
        ...
        public void OnPost()
        {
            ...
            if (action == "done")
            {
                if (!user_progress.module1)
                {
                    ...
                    string connectionString1 = "Server=localhost;Database=elearn;User=root;Password=;";
                    String sql = "UPDATE progress SET module2 = @v WHERE id_user = @id";
                    using (MySqlConnection connection = new MySqlConnection(connectionString1))
                    {
                        using (MySqlCommand command = new MySqlCommand(sql, connection))
                        {
                            ...
                            connection.Open();
                            int rowsAffected = command.ExecuteNonQuery(); // ExecuteNonQuery directly

                            if (rowsAffected > 0)
                            {
                                error = "Congratulations! You completed the module.";
                            }
                            else
                            {
                                error = "Module progress update failed.";
                            }
                ...
                {
                    error = "Youe have already submit this module !";
                }
